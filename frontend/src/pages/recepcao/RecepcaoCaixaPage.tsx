/**
 * Operações de caixa na recepção (abrir, fechar, sangria, suprimento).
 */
...
export default function RecepcaoCaixaPage() {
  ...
  const carregar = async () => {
    const { data } = await recepcaoApi.caixaAtual()
    setCaixa(data)
  }
  ...
  const movimento = async (e) => {
    ...
    try {
      ...
      if (movOpen === 'sangria') await recepcaoApi.sangria(payload)
      else await recepcaoApi.suprimento(payload)
      setMovOpen(null)
      setMovValor('')
      setMovDesc('')
      await carregar()
